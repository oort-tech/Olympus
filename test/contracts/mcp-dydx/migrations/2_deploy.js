...
async function deployBaseProtocol(deployer, network) {
  ...
  console.log('solo margin deployed')
...
console.log('adminimpl & operation impl deployed')
...
console.log('deployed polynomialinterest & doubleExponent');
...
async function deploySecondLayer(deployer, network, accounts) {
  ...
  if (isDevNetwork(network) || isHuygensDev(network)) {
    await Promise.all([
...
async function getSoloMargin(network) {
  if (isDevNetwork(network) || isHuygensDev()) {
