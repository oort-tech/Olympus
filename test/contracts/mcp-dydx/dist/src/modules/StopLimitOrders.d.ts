...
import { CallOptions, Decimal, Integer, StopLimitOrder, SignedStopLimitOrder, LimitOrderState, SigningMethod } from '../../src/types';
export declare class StopLimitOrders extends OrderSigner {
    private networkId;
     ...
     */
    getOrderStates(orders: StopLimitOrder[], options?: CallOptions): Promise<LimitOrderState[]>;
    /**
     ...
     */
    getAccountCollateralizationAfterMakingOrders(weis: Integer[], prices: Integer[], orders: StopLimitOrder[], remainingMakerAmounts: Integer[]): Decimal;
    /**
     * Returns the final signable EIP712 hash for approving an order.
     */
    getOrderHash(order: StopLimitOrder): string;
    /**
    ...
    unsignedOrderToBytes(order: StopLimitOrder): string;
    signedOrderToBytes(order: SignedStopLimitOrder): string;
    private orderToByteArray;
    private getDomainData;
    protected ethSignTypedOrderInternal(order: StopLimitOrder, signingMethod: SigningMethod): Promise<string>;
    ...
    protected getContract(): import("../../build/wrappers/StopLimitOrders").StopLimitOrders;
}
