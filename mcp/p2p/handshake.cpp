...
void mcp::p2p::hankshake::writeInfo()
{
	...
	hankshake_msg handmsg(m_host->id(), mcp::p2p::version, mcp::mcp_network, caps, m_ecdheLocal.pub_comp());
	RLPStream s;
...
void hankshake::writeAuth()
{
	...
	m_ecdheLocal.pub_comp().ref().copyTo(pubk);
...
void hankshake::transition(boost::system::error_code _ech)
{
	...
	else if (m_nextState == WriteHello)
	{
		...
		hankshake_msg handmsg(m_host->id(), mcp::p2p::version, mcp::mcp_network, m_host->caps(), m_ecdheLocal.pub_comp());
		RLPStream s;
