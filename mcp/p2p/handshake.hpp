...
namespace mcp
{
	namespace p2p
	{
		...
		{
		public:
			hankshake_msg(node_id const & node_id_a, uint16_t const & version_a, mcp::mcp_networks const & network_a, std::list<capability_desc> const & cap_descs_a, dev::PublicCompressed const & pubkey_a);
		...
		private:
			...
			void setAuthValues(dev::Signature const& sig, dev::PublicCompressed const& _hePubk, dev::PublicCompressed const& remotePubk, mcp::nonce const& remoteNonce);
			...
			dev::PublicCompressed m_ecdheRemote;			///< Remote ephemeral public key.
