	...
	m_store(store_a)
{
	...
	for (; it.valid(); ++it)
	{
		auto account(mcp::slice_to_account(it.key()));
...
mcp::account mcp::key_manager::create(std::string const & password_a, bool const & is_backup_a)
{
	mcp::raw_key prv;
...
bool mcp::key_manager::change_password(mcp::account const & account_a,
...
bool mcp::key_manager::decrypt_prv(mcp::account const & account_a, std::string const & password_a, mcp::raw_key & prv)
{
	bool error(false);
	mcp::key_content kc;
	{
