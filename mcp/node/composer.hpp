...
namespace mcp
{
	...
	public:
		...
		~composer();
		mcp::compose_result compose_joint(mcp::db::db_transaction & transaction_a, mcp::block_type const & type_a, boost::optional<mcp::block_hash> const & previous_a, mcp::account const & from_a, mcp::account const & to_a, mcp::amount const & amount_a, uint256_t const & gas_a, uint256_t const & gas_price_a, std::vector<uint8_t> const & data_a, mcp::raw_key const & prv_a, mcp::public_key const & pub_a);
		...
		mcp::compose_result sign_and_compose_joint(std::shared_ptr<mcp::block> block_a, mcp::signature const & signature_a);
