	...
	mcp::hash_tree_response_message & hash_tree_response)
{
	...
	for (uint64_t index = from_index; index <= to_index; index++)
	{
		...
		mcp::hash_tree_response_message::summary_items s(bh, sh, previous_summary, p_summaries, bs->m_receiptsRoot, s_summaries, bs->status, bs->stable_index, bs->mc_timestamp, bs->level, block_ptr, trannsactions, approves, *bs->main_chain_index, s_info.list);
