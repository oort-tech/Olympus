...
mcp::chain::chain(mcp::block_store &store_a) : m_store(store_a),
											   m_stopped(false)
{
}
		...
		{
			{
				...
				{
					assert_x_msg(false, "block exists, did not added count, hash:" + hash.hex());
				}
...
void mcp::chain::add_new_witness_list(mcp::db::db_transaction &transaction_a, uint64_t mc_last_summary_mci)
{
	...
	if (old_elected_epoch == elected_epoch)
		return;
	else
		old_elected_epoch = elected_epoch;
...
// test switch to all new witness
	...
	if (vrf_outputs.find(elected_epoch) == vrf_outputs.end())
		return;
...
void mcp::chain::write_dag_block(mcp::db::db_transaction &transaction_a, std::shared_ptr<mcp::process_block_cache> cache_a, std::shared_ptr<mcp::block> block_a)
{
	...
	assert_x(last_summary_block_state && last_summary_block_state->is_stable && last_summary_block_state->is_on_main_chain && last_summary_block_state->main_chain_index);
										 ...
										 bool &is_mci_retreat, uint64_t &retreat_mci, uint64_t &retreat_level, std::list<mcp::block_hash> &new_mc_block_hashs)
{
	...
	{
		std::string msg(boost::str(boost::format("stable mci retreat, last added block: %1%, retreat mci: %2%, last stable mci: %3%") % block_a->hash().hex() % retreat_mci % m_last_stable_mci_internal));
		LOG(m_log.info) << msg;
...
void mcp::chain::send_approve(std::shared_ptr<ApproveQueue> aq_a)
{
	...
	std::array<CryptoPP::byte, 65> serializedPubkey;
...
//Using it here so we don't get a linker error
//on mcp::init
template <class DB>
