...
namespace mcp
{
    ...
    struct Change
    {
        ...
        Change(Kind _kind, Address const &_addr, uint256_t const &_value = 0) : kind(_kind), address(_addr), value(_value)
        {
        ...
        Change(Address const &_addr, uint256_t const &_key, uint256_t const &_value) : kind(Storage), address(_addr), value(_value), key(_key)
        {
        }

        /// Helper constructor for nonce change log.
        Change(Address const &_addr, uint256_t const &_value) : kind(Nonce), address(_addr), value(_value)
        {
        }

        /// Helper constructor especially for new code change log.
        Change(Address const &_addr, dev::bytes const &_oldCode) : kind(Code), address(_addr), oldCode(_oldCode)
        {
        }
    };
