...
public:
    ...
    owning_bytes_ref exec(const evmc_host_interface* _host, evmc_host_context* _context,
        evmc_revision _rev, const evmc_message* _msg, uint8_t const* _code, size_t _codeSize, std::shared_ptr<EVMLogger> _tracer = nullptr);

    uint64_t m_io_gas = 0;
public:
    //bytes const& getMemory() const { return m_newMemSize; }
